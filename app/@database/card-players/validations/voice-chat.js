  ...
  try {
    ...
    if (!validator.isBoolean(data)) {
      throw new CustomError({
        level: "warn",
        errorsArr: [{ code: "jma2KUQIg", messageID: "qnWsuPcrJ" }],
      });
    }
  } catch (errorObj) {
    // ---------------------------------------------
 ...
 * @return {Object} バリデーション結果
 */
const validationCardPlayersVoiceChatComment = ({
  throwError = false,
  required = false,
  value,
}) => {
  ...
  try {
    ...
    if (validator.isEmpty(data)) {
      if (required) {
        throw new CustomError({
          level: "warn",
          errorsArr: [{ code: "o25q3UXUy", messageID: "cFbXmuFVh" }],
        });
      }
    ...
    if (!validator.isLength(data, { min: minLength, max: maxLength })) {
      throw new CustomError({
        level: "warn",
        errorsArr: [{ code: "iUqrQ7Mms", messageID: "pLES2ZGM2" }],
      });
    }
  } catch (errorObj) {
    // ---------------------------------------------
